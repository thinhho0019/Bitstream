...
export default function TableComponent({ label, value, onDelete }: Props) {
    ...
    return (
        <div className="flex flex-col mt-4 ">
            ...
            <div className="max-h-[350px] overflow-y-auto overflow-x-auto w-full border border-gray-200 dark:border-neutral-600 shadow-md rounded-lg">
                        ...
                        {value.map((row, idx) => (
                            <tr key={idx}>
                                {keys.map((key) => {
                                    ...
                                    return (
                                        <td
                                            ...
                                            className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200 text-center"
                                        >
                                            {row[key as keyof AssetPrediction]}
                                        </td>
                                    )
                                })}
                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <button
                                        type="button"
                                        onClick={() => setSelectedDeleteIndex(row['id'] as number)}
