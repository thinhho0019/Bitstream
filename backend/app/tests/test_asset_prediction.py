...
from app.tests.conftest import override_get_db_with_model, client, override_get_db_with_models
...
def init_mock():
    fake_account = Account(id=str(user_id), email="user@example.com", name="User", image="", password="",
                           login_sessions=[])
    fake_message = Message(sender="user", content="", account_id=user_id, timestamp="timetime")
    ...
    app.dependency_overrides[get_db] = override_get_db_with_models(fake_account, fake_message, fake_asset_prediction)
...
def test_create_asset_success(mock_rdBase):
    init_mock()
    response = client.post("/api/asset-predictions", json={
        "name": "bitcoin",
        "current_value": 100000,
        "next_value": 120000,
        "expiration_time": "1H",
